...
namespace LineGame
{
    public class DrawLine : SingletonMonoBehaviour<DrawLine>
    {
        ...
        public void ToggleEnable()
        {
            ...
            {
                UI_Ingame_R.Instance.lineToggleCheck.gameObject.SetActive(true);
                enableDraw = true;
            }
            else if (activeToggle.isOn == false)
            {
                UI_Ingame_R.Instance.lineToggleCheck.gameObject.SetActive(false);
                enableDraw = false;
            }
        }
        public void SceneInitialized()
        {
            maxInk_line = MissionObjectList.Instance.objectList.BuildIndexToLevelIndex().Ink_Line;
            cam = null;
            activeToggle = null;

            cam = Camera.main;
            //토글 버튼 클릭으로 펜 그리기 말기
            activeToggle = UI_Ingame_R.Instance.lineToggle;
            //그릴 수 없는 레이어 지정
            //----------------------------------------------------------------
