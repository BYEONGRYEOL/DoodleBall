...
namespace LineGame
{
    public class DrawLineUp : SingletonMonoBehaviour<DrawLineUp>
    {
        ...
        void BeginDraw()
        {
            ...
            currentLine = Instantiate(linePrefab, this.transform).GetComponent<LineUp>();
        ...
        public void ToggleEnable()
        {
            ...
            {
                UI_Ingame_R.Instance.lineUpToggleCheck.gameObject.SetActive(true);
                enableDraw = true;
            }
            else if (activeToggle.isOn == false)
            {
                UI_Ingame_R.Instance.lineUpToggleCheck.gameObject.SetActive(false);
                enableDraw = false;
            }
        }
        public void SceneInitialized()
        {
            maxInk_line = MissionObjectList.Instance.objectList.BuildIndexToLevelIndex().Ink_LineUp;
            ...
            cam = Camera.main;
            //토글 버튼 클릭으로 펜 그리기 말기
            activeToggle = UI_Ingame_R.Instance.lineUpToggle;
            //그릴 수 없는 레이어 지정
            //----------------------------------------------------------------
